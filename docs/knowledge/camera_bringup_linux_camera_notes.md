...
- IMX415 sensor 被内核识别：`m00_b_imx415 5-0037`。
- MIPI 链路建立：`IMX415 -> rockchip-csi2-dphy0 -> rockchip-mipi-csi2 -> rkcif-mipi-lvds1 -> rkisp-vir1`。
- 应用层推荐采集节点：`/dev/video42`。
- `/dev/video42` 是 `rkisp-vir1` 的 ISP mainpath，输出 `NV12 3840x2160`。
- 已通过 `v4l2-ctl --stream-mmap` 连续抓取 NV12/YUV 数据。
...
所以 `/dev/video42` 并不是“凭空出现”的文件，而是内核中某个 V4L2 video device 注册后由系统创建设备节点。
...
- `5-0037` 表示 I2C bus 5 上地址 `0x37` 的设备。
...
这些不是当前链路失败，而是驱动/设备树尝试了多个可能位置。当前最终有效的是 `5-0037`。
...
这也是为什么我们同时看到 `/dev/video11` 和 `/dev/video42`：

- `/dev/video11` 是 rkcif 的 raw Bayer 输出，格式是 `GB10`。
- `/dev/video42` 是 rkisp mainpath 输出，格式是 `NV12`。

第一版应用更适合使用 `/dev/video42`，因为 OpenCV/显示/人脸识别处理 YUV/NV12 通常比处理 raw Bayer 更直接。
...
```bash
v4l2-ctl -d /dev/video42 --stream-mmap --stream-count=10 --stream-to=/tmp/video42_nv12.yuv
```

用 mmap 方式连续抓取 NV12/YUV 数据，并写入未压缩帧文件。
...
```text
3840 x 2160 x 1.5 x 10 = 124416000 字节
```
...
注意：raw/YUV 文件不是图片文件，不能直接双击查看。后续如果要可视化，需要按格式转换为 PNG/JPG。
...
这种设备节点列表对应用层选择采集入口很有帮助。如果系统提供 `/dev/video-camera*` 这类别名，也应继续追踪它最终指向哪个真实 `/dev/video*` 节点。
...
这就是我们后面能判断 `/dev/video42` 更适合应用层的原因。

### 9.9 为什么遍历所有 `/dev/media*`
...
这次换 MIPI 接口后，关键经验就是不能只看默认 `media-ctl -p`。默认输出通常只对应某一个 media controller，例如旧接口可能在 `/dev/media0`，而新接口有效链路实际在 `/dev/media1` 和 `/dev/media4`。

遍历所有 `/dev/media*` 可以直接确认：

- 哪个 media graph 里有真正的 `m00_b_imx415 5-0037`。
- 哪条链路的 link 是 `[ENABLED]`。
- raw CIF 节点和 ISP mainpath 节点分别是谁。
- 换接口后应用层节点是否从旧的 `/dev/video33` 变成新的 `/dev/video42`。
...
后面确认 `/dev/video42` 后，才手动执行抓帧命令，这是更安全的流程。
...
2. 通过 `dmesg` 确认 IMX415 在 `i2c-5@0x37` 被 probe 成功，并且和 Rockchip MIPI DPHY 匹配。
3. 通过遍历 `/dev/media*`，确认最终链路是 `IMX415 -> CSI DPHY -> MIPI CSI2 -> rkcif-mipi-lvds1 -> rkisp-vir1 -> /dev/video42`。
4. 通过 `v4l2-ctl --list-devices` 和 `--list-formats-ext` 区分 raw Bayer 节点和 ISP 输出节点。
5. 最终选择 `/dev/video42` 作为后续应用层输入，因为它是 `rkisp-vir1` 的 `rkisp_mainpath`，能输出 `NV12`。
6. 通过 NV12/YUV 抓帧验证，文件大小与 `3840x2160 NV12` 理论大小一致，证明采集链路可用。
